...
// axios.get() function takes a URL and returns a promise.
axios.get(queryURL).then(function(response) {
  console.log(response.data.venue.name \n response.data.venue.city + ", " +
    response.data.venue.region \n response.data.datetime)
});
...
for an artist and render the following information about each event to the
terminal:
...
The Spotify API requires you sign up as a developer to generate the necessary
credentials. You can follow these steps in order to generate a client id and client
secret:
...
https://developer.spotify.com/my-applications/#!/applications/create to register a
new application to be used with the Spotify API. You can fill in whatever you'd
like for these fields. When finished, click the "complete" button.

Step Four: On the next screen, scroll down to where you see your client id and
client secret. Copy these values down somewhere, you'll need them to use the
Spotify API and the node-spotify-api package. */
...
You'll use the axios package to retrieve data from the OMDB API. Like all of the
